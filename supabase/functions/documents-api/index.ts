...
serve(async (req) => {
  ...
  try {
    // Получаем authorization header для проверки пользователя
    ...
    // Инициализация Supabase клиента с SERVICE_ROLE_KEY для обхода RLS
    ...
    // Проверка аутентификации - передаем JWT явно
    const {
      data: { user },
      error: authError,
